...
const getImgUrlById = (imgId: number, w = 200, h = 300): string =>
  `${API_IMAGES_BASE}/id/${imgId}/${w}/${h}.jpg`
const getImgNameById = (imgId: number, w = 200, h = 300): string =>
  `id-${imgId}-${w}-${h}.jpg`
...
export const genPhotoImage = async (): Promise<string> => {
  ...
  response.data.pipe(
    fs.createWriteStream(path.join(__dirname, pathToData, imgName))
  )
